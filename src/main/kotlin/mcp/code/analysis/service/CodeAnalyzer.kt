...
class CodeAnalyzer() {
  companion object {
    const val BINARY_FILE_SIZE_THRESHOLD: Long = 1024 * 1024 // 1MB
    const val BINARY_DETECTION_THRESHOLD: Double = 0.05
    ...
    private fun analyzeFile(file: File): Map<String, Any> {
      ...
      when {
        fileSize > BINARY_FILE_SIZE_THRESHOLD ->
          return mapOf("size" to fileSize, "extension" to file.extension, "skipped" to "File too large")

        isBinaryFile(file) ->
          return mapOf("size" to fileSize, "extension" to file.extension, "skipped" to "Binary file")
      }
    ...
    private fun isBinaryFile(file: File): Boolean {
      ...
      return try {
        ...
        nullCount > bytes.size * BINARY_DETECTION_THRESHOLD
      } catch (e: Exception) {
        false
      }
    }
