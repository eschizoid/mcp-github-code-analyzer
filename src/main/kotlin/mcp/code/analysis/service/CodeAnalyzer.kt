...
object CodeAnalyzer {
  ...
  private fun processDirectory(dir: File, rootPath: String): Map<String, Any> {
    ...
    return files.associate { file ->
      ...
      if (file.isDirectory) relativePath to processDirectory(file, rootPath) else relativePath to analyzeFile(file)
    }
  }
  ...
  private fun analyzeFile(file: File): Map<String, Any> {
    ...
    return try {
      ...
      extractDeclarations(content, language).forEach { (k, v) -> result[k] = v }
      result
    } catch (e: Exception) {
  ...
  private fun extractImports(content: String, language: String): List<String> =
    when (language) {
      "kotlin" -> Regex("import\\s+([\\w.]+)(?:\\s+as\\s+\\w+)?.*").findAll(content).map { it.groupValues[1] }.toList()
      "java" ->
        Regex("import\\s+(?:static\\s+)?([\\w.]+)(?:\\.[*])?;").findAll(content).map { it.groupValues[1] }.toList()
      "python" -> {
        ...
        val imports =
          importRegex.findAll(content).flatMap { it.groupValues[1].split(",").map { s -> s.trim().split(".").first() } }
      ...
      "go" -> {
        ...
        val multiImports =
          multiImportRegex.findAll(content).flatMap {
            Regex("[\"']([\\w./]+)[\"']").findAll(it.groupValues[1]).map { m -> m.groupValues[1] }
          }
        (singleImports + multiImports).toList()
      }
      "ruby" ->
        Regex("require(?:_relative)?\\s+['\"]([^'\"]+)['\"]").findAll(content).map { it.groupValues[1] }.toList()
      "scala" -> Regex("import\\s+([\\w.{}=>,$\\s]+)").findAll(content).map { it.groupValues[1].trim() }.toList()
      "rust" -> Regex("use\\s+([\\w:]+)(?:::\\{[^}]*\\})?;").findAll(content).map { it.groupValues[1] }.toList()
      "php" -> {
  ...
  private fun extractDeclarations(content: String, language: String): Map<String, List<String>> {
    ...
    when (language) {
      ...
      "java" -> {
        ...
        val classRegex = Regex("(?:class|interface|enum)\\s+(\\w+)(?:<[^>]*>)?[^{]*\\{")
        declarations["classes"] = classRegex.findAll(content).map { it.groupValues[1] }.toList()
        // Methods
        val methodRegex =
          Regex(
            "(?:public|protected|private|static|final|synchronized|abstract|native|transient|\\s)+[\\w\\[\\]<>]+\\s+(\\w+)\\s*\\("
          )
      ...
      "typescript" -> {
        ...
        val functionRegex = Regex("function\\s+(\\w+)\\s*\\(|const\\s+(\\w+)\\s*=\\s*\\([^)]*\\)\\s*=>")
        declarations["functions"] =
          functionRegex.findAll(content).flatMap { it.groupValues.drop(1).filter { n -> n.isNotEmpty() } }.toList()
        // For TypeScript: interfaces and types
        if (language == "typescript") {
          val interfaceRegex = Regex("interface\\s+(\\w+)\\s*\\{")
      ...
      "rust" -> {
        ...
        // Functions and methods
        val functionRegex = Regex("fn\\s+(\\w+)\\s*\\(")
        declarations["functions"] = functionRegex.findAll(content).map { it.groupValues[1] }.toList()
        // Traits (interfaces)
      ...
      "c" -> {
        ...
        declarations["classes"] = classRegex.findAll(content).map { it.groupValues[1] }.toList()
        // Functions
        val functionRegex = Regex("(?:[\\w\\*&]+\\s+)+([\\w_][\\w\\d_]*)\\s*\\([^)]*\\)\\s*\\{")
        declarations["functions"] = functionRegex.findAll(content).map { it.groupValues[1] }.toList()
      }
    }
    return declarations
  ...
  private fun findCodeFiles(dir: File): List<File> =
    if (dir.isHidden || dir.name == ".git") emptyList()
    else
