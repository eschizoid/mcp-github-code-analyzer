...
class ModelContextService {
    ...
    suspend fun generateResponse(prompt: String): String {
        try {
            ...
            val httpResponse = client.post(ollamaApiUrl) {
    ...
    fun collectCoreCodeSnippets(repoDir: File): List<String> {
        ...
        val coreFiles = findCodeFiles(repoDir).filter { file ->
    ...
    fun collectImportantFiles(repoDir: File): List<String> {
        ...
        // Find main file if it exists
        val possibleMainFiles = findCodeFiles(repoDir).filter { file ->
            file.name.equals("Main.kt", ignoreCase = true) ||
                    file.name.startsWith("Application")
        }
        importantFiles.addAll(possibleMainFiles.take(2))

        // Add a few more interesting files
        val otherFiles = findCodeFiles(repoDir)
    ...
    private fun isCodeFile(file: File): Boolean {
        val codeExtensions = setOf(
            // JVM languages
            "kt", "java", "scala", "groovy", "clj",

            // Script languages
            "py", "rb", "js", "jsx", "ts", "tsx", "php", "pl", "pm", "sh", "bash", "ps1",

            // Systems programming
            "c", "cpp", "cc", "cxx", "h", "hpp", "cs", "go", "rs", "swift", "m",

            // Web
            "html", "htm", "css", "scss", "sass", "less", "vue", "svelte",

            // Data formats & Config
            "json", "xml", "yaml", "yml", "toml", "properties", "ini", "conf", "md",

            // Build files
            "gradle", "gradle.kts", "sbt", "pom", "cmake", "make", "mk",

            // Other languages
            "sql", "r", "dart", "lua", "ex", "exs", "erl", "hrl", "hs", "fs", "fsx", "jl"
        )
